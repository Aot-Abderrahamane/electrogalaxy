...
class IndexController extends Controller
{
	...
	public function index()
	{
		...
		$special_offer = Product::where('special_offer', 1)->orderBy('product_name', 'DESC')->limit(6)->get();
	...
	public function UserProfileStore(Request $request)
	{
		...
		return redirect()->route('dashboard')->with("message", "Profile mis à jour avec succès");
	}
	...
	public function ProductDetails($id, $slug)
	{
		...
		$breadproduct = SubCategory::with(['category'])->where('id', $subcat_id)->get();
		$breadproduct1 = SubSubCategory::with(['category', 'subcategory'])->where('id', $subsubcat_id)->get();

		//afficher sizes et les couleurs des produits 
		$color = $product->product_color;
		$product_color = explode(',', $color);
		...
		return view('frontend.product.product_details', compact('product', 'product_color', 'product_size', 'multiImag', 'product_color', 'breadproduct', 'breadproduct1', 'product_size', 'relatedProduct'));
	}
